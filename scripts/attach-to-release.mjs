...
/**
 * Attaches a built file to the release of a tag, draft or not.
 *
 * The release the Windows job drafts is not visible through `releases/tags`, which only
 * answers for published ones, so the release is found by listing them instead.
 *
...
const releases = await fetch(`https://api.github.com/repos/${repo}/releases?per_page=30`, {
  headers,
}).then(asJson);
const release = releases.find((each) => each.tag_name === tag);
if (!release) throw new Error(`no release for ${tag}`);

const name = basename(file);
const already = release.assets.find((asset) => asset.name === name);
if (already) {
  await fetch(`https://api.github.com/repos/${repo}/releases/assets/${already.id}`, {
    method: "DELETE",
    headers,
  });
}
