...
- **A counter ticked by a timer.** Add a second every second. Wrong the moment the page is
  hidden, the laptop sleeps, or the tab is throttled — and browsers throttle background
  timers on purpose.
...
- Sleep, a closed window, and a reboot cost nothing: the difference is still right.
