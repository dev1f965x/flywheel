...
The point of the Discord part is that friends see what someone is working on. Discord
offers two ways for something outside Discord to say that, and they are not alike.
...
- **The account's own token.** A program pretends to be the person's Discord client. It
  would work from anywhere, including a phone, and it is against Discord's terms and gets
  accounts banned. Not an option at any price.
- **A bot posting to a channel.** A different thing entirely: it needs a server, a bot, an
  invite, and it writes messages nobody asked for.
...
- Discord being closed is normal, not an error. The connection is retried quietly and the
  window says whether it is connected.
